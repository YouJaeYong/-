...
void Send_login_packet(wchar_t* name, int id)
{
	...
	wchar_t weapon[10] = { 0 };
	swprintf_s(query, L"SELECT c_name, c_xpos, c_ypos, c_level, c_power, c_weapon, c_exp, c_hp FROM [M.O.GameServerExam-DB].[dbo].[C_Table1] WHERE c_name = '%s' ", name);
	swprintf_s(insert_query, L"INSERT INTO [M.O.GameServerExam-DB].[dbo].[C_Table1] VALUES('%s', %d, %d, %d, %d, '%s', %d, %d)",
	...
	if (retcode == SQL_SUCCESS || retcode == SQL_SUCCESS_WITH_INFO)
	{
		...
		if (retcode == SQL_SUCCESS || retcode == SQL_SUCCESS_WITH_INFO)
		{
			...
			if (retcode == SQL_SUCCESS || retcode == SQL_SUCCESS_WITH_INFO)
			{
				...
				retcode = SQLConnect(hdbc, (SQLWCHAR*)L"M.O.GameServerExam-DB", SQL_NTS, (SQLWCHAR*)NULL, 0, NULL, 0);
...
void ClientDIsconnect(int key)
{
	...
	wchar_t query[200] = { 0 };
	swprintf_s(query, L"UPDATE [M.O.GameServerExam-DB].[dbo].[C_Table1] SET c_xpos = %d , c_ypos = %d , c_level = %d, c_power = %d, c_weapon = '%s', c_exp = %d , c_hp = %d WHERE c_name = '%s' ",
